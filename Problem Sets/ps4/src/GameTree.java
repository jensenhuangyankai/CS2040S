...
public class GameTree {
    ...
    int recurseNode(TreeNode node, int A) { // A represents if it is A or B node -- 1 == A 0 == B
        if (node.leaf) return node.value;
        int nodeValue = (A == 1) ? Integer.MIN_VALUE : Integer.MAX_VALUE;
        for (TreeNode child : node.children){
            if (A == 1){
                nodeValue = Math.max(nodeValue, recurseNode(child, 0));
            }
            else{
                nodeValue = Math.min(nodeValue, recurseNode(child, 1));
            }
        }
